...
int ProcessTXT1(char* fname) // обчислення загальної кількостіслів ,які починаються з букви “b”  =
{
    ifstream fin(fname); // відкрили файл для зчитування
    string s; // прочитаний рядок
    int k = 0; // загальна кількість символів ,які починаються з букви “b” =
    while (getline(fin, s)) // поки можна прочитати рядок
    { // скануємо його і обчислюємо кількість слів ,які починаються з букви “b”  =
